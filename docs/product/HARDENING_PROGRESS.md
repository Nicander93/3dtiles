...
| H14 | 代码完成，资源曲线已取得 | 磁盘、线程、峰值内存指标已接入；1/2/4 worker 真实样本曲线均成功；约 932 MB/4567 Tile 样本取消后无残留；香港 8×8（约 2.47 GB、11,124 OSGB）2 worker 全量转换成功；真实 ACL/磁盘满故障注入仍待执行 |
| H15 | CI 门槛完成，安装验收部分完成 | `npm ci`、前端测试/构建、Rust/Tauri、NSIS 临时安装、带空格安装路径、真实 OSGB 与中文路径、10 次重复和香港 8×8 候选 runtime 回归已通过；Cesium、当前用户失败样本、完整桌面关闭/多级进程树仍待执行 |
...
- 使用当前 processor 对已有连续香港 5×4 真实成果（4,585 个内容文件、约 1.23 GB）完成 `rebuild-top --levels 0`→validate→commit；耗时 29,034 ms，提交 4,596 个文件，生成 9 个 proxy，层级计数 20/6/2/1，并保留既有几何警告（回归 34）。
...
- 真实中文输入根目录已通过；本地 Cesium 页面已加载真实 `OSGBny` 成果并显示 `tilesLoaded`（回归 24）；带空格的安装目录 convert-only、方括号/加号/括号特殊路径已通过（回归 31–32），长路径已得到可诊断拒绝（回归 33），4,585 文件香港成果的重建链和 11,124 OSGB 的 8×8 全量转换也已通过（回归 34–35）。中文 Tile 文件名、权限/磁盘故障和安装桌面 WebView 仍待端到端回归。
