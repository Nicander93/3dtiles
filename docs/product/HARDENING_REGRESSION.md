...
| 13 | converter 本机 Release CLI | MSVC/vcpkg 原生构建与启动 | `cargo build --release`；`_3dtile.exe --help` | `D:\\code\\3dtiles\\dist\\runtime` | 通过 | 0 | stdout/stderr | MSVC 14.51.36231；包含空 OSGB 叶节点修复的候选 hash `af7384f15059d6c3f6927f1aa754bffcf2036504fce3cb3628eb536c77e8e81e` |
...
| 36 | processor 本机 Release runtime | 香港 8×8 转换成果执行完整顶层重建 | `processor.exe process-tileset --input .cache\\hk8x8-convert-fixed-t2-20260916 --output <new path> --rebuild-top --levels 0 --texture keep --task-id hk8x8-rebuild-top-20260917` | `.cache\\hk8x8-rebuild-top-20260917` | 通过 | 0 | `.cache\\hk8x8-rebuild-top-20260917.jsonl` | 明确记录 `rebuild→validate→commit→succeeded`；耗时 72,205 ms，输出 11,208 个文件、3,145,536,788 bytes，root children=4、节点 85、proxy=21，85/85 个 content URI 可解析；无残留 processor/top_rebuild 进程 |
| 37 | 本机候选 NSIS 安装 | 安装包携带最新 converter runtime；安装后真实转换与卸载 | 安装到 `.cache\\installed-candidate-20260917c`；校验 `_3dtile.exe` SHA256；执行 ASCII `OSGBny` convert-only；静默卸载 | `.cache\\installed-candidate-output-20260917c` | 通过 | 0（安装/转换/卸载） | `.cache\\installed-candidate-20260917c.log` | 安装内 converter hash=`af7384f15059d6c3f6927f1aa754bffcf2036504fce3cb3628eb536c77e8e81e`，与候选一致；转换生成 85 个文件和 `tileset.json`；卸载退出 0 且安装目录已移除；该次安装目录无空格，带空格安装路径沿用回归 31 |
