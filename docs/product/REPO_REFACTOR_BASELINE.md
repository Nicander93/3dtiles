...
日期：2026-09-13  
HEAD：`c1e49c356e9903251255311bc80a743847566737`（工作区另有未提交 UI 改动，须保留）
...
| 真实 OSGB 样例 | 仓库内无 OSGB 数据；tiles 有 `tests/fixtures/top_rebuild/*` |
...
- 转换器：根 package `_3dtile`（Cargo + CMake/vcpkg）
...
| commands 内复制 scan | → 调用 `processor scan-osgb` |
| Python HTTP 任务回退 | 正式路径移除；缺 processor 明确失败 |
| 根 `_3dtile` 源码/CMake/vcpkg | → `engines/3dtiles-converter/` |
...
- Processor 缺失时当前会静默走 Python：原有设计缺陷 → 本轮改为显式失败
- commit 覆盖已有输出：原有缺陷 → T01
- 取消杀单 PID：原有缺陷 → T03
