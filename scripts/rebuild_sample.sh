...
if [[ ! -x "$PY" ]]; then
  ...
  "$ROOT/.venv/bin/pip" install -r "$ROOT/tools/rebuild_top/requirements.txt"
  PY="$ROOT/.venv/bin/python"
fi
...
"$PY" "$ROOT/tools/rebuild_top/rebuild_top.py" -i "$IN" -o "$OUT" --levels 1 --simplify 0.5 --texture-scale 0.5 -v
