...
def preview_osgb_native(body: PathRef) -> Dict[str, Any]:
    """Spawn native Qt/OSG viewer for an OSGB dataset root."""
    path = Path(body.path).expanduser()
    if not path.exists():
        raise HTTPException(400, f"path does not exist: {path}")
    script = REPO / "scripts" / "run_osgb_viewer.sh"
    viewer_bin = REPO / "apps" / "osgb_viewer" / "build" / "osgb_viewer"
    if not script.is_file() and not viewer_bin.is_file():
        raise HTTPException(500, "osgb_viewer not built; see apps/osgb_viewer/README.md")
    display = os.environ.get("DISPLAY", ":2")
    env = os.environ.copy()
    env["DISPLAY"] = display
    cmd = [str(script), str(path)] if script.is_file() else [str(viewer_bin), str(path)]
    try:
        ...
        raise HTTPException(500, f"failed to spawn viewer: {e}") from e
    ...
    try:
        ...
        return {
            "ok": False,
            "error": f"viewer exited immediately rc={rc}: {err.decode('utf-8', errors='replace')[-500:]}",
            "display": display,
        }
    except subprocess.TimeoutExpired:
        pass
    return {"ok": True, "pid": proc.pid, "display": display, "path": str(path)}
...
def index() -> FileResponse:
    ...
    raise HTTPException(404, "apps/web/dist/index.html not found")
...
def spa_or_static(full_path: str) -> FileResponse:
    ...
    if index_html.is_file():
        return FileResponse(index_html)
    raise HTTPException(404, "apps/web/dist not found")
