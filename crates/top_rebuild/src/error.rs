...
pub enum TopRebuildError {
    ...
    /// Hardening / KTX2 path (maps semantically to CONTENT_INVALID).
    #[error("CONTENT_UNREADABLE: {0}")]
    ContentUnreadable(String),

    /// Hardening path for unsupported mesh/content kinds.
...
impl TopRebuildError {
    ...
    pub fn code(&self) -> Option<ErrorCode> {
        match self {
            ...
            Self::ContentUnreadable(_) => Some(ErrorCode::ContentInvalid),
            Self::UnsupportedContent(_) => Some(ErrorCode::ContentInvalid),
            _ => None,
        }
    }
    ...
    pub fn code_str(&self) -> &str {
        match self {
            ...
            Self::ContentUnreadable(_) => "CONTENT_UNREADABLE",
