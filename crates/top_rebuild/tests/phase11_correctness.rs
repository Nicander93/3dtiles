...
fn release_missing_content_must_fail() {
    ...
    assert!(
        msg.contains("CONTENT_MISSING") || msg.contains("missing"),
        "got {msg}"
    );
}
...
fn release_corrupt_b3dm_must_fail() {
    ...
    assert!(
        msg.contains("CONTENT_INVALID")
            ...
            || msg.contains("CONTENT_UNREADABLE")
            || msg.contains("invalid")
            || msg.contains("glb magic"),
        "got {msg}"
    );
}
...
fn debug_fixture_may_synthesize_only_when_explicit() {
    ...
    let msg = err.to_string();
    assert!(msg.contains("CONTENT") || msg.contains("missing"), "{}", err);
}
...
fn subtree_preservation_grid_4x4() {
    ...
    if src.exists() && dst.exists() {
        ...
        assert_eq!(sn, on, "node count preserved");
        assert_eq!(sc, oc, "content count preserved");
        assert_eq!(ss, os, "URI stems preserved");
    }
}
...
fn transform_identity_translation_rotation_nested() {
    ...
    let back = ecef
        .inverse()
        .unwrap()
        .transform_point(-2_556_000.0, 4_780_000.0, 3_350_000.0);
